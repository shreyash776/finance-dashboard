...
function extractFields(obj: unknown, prefix = '', maxDepth = 2, currentDepth = 0): string[] {
...
function truncateData(data: unknown): unknown {
  ...
  if (dataString.length > 1000) {
    const truncated = JSON.parse(dataString.slice(0, 1000));
    return { ...truncated, '...': 'Data truncated for preview' };
  }
  return data;
}
